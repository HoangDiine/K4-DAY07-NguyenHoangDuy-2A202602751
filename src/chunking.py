...
class SentenceChunker:
    ...
    def chunk(self, text: str) -> list[str]:
        if not text or not text.strip():
            return []
        # ponytail: (?<=[.!?])\s+ preserves punctuation; abbrevs (TS., v.v.) and decimals split incorrectly.
...
class RecursiveChunker:
    ...
    def _split(self, current_text: str, remaining_separators: list[str]) -> list[str]:
        ...
        splits = [s for s in current_text.split(sep) if s]
        if not splits:
            return []
        ...
        for s in splits:
            if len(s) > self.chunk_size:
                docs.extend(self._split(s, next_separators))
            else:
                docs.append(s)
        ...
        for doc in docs:
            if not current:
                current = doc
            elif len(current) + len(sep) + len(doc) <= self.chunk_size:
                current += sep + doc
            else:
